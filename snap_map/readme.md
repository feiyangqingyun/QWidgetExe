...
2. 可执行文件在源码同级目录下的bin目录，和core_map目录同级。
...
4. **离线地图目录可能有三个文件夹，tiles表示街道图，tiles_hybrid表示卫星图，tiles_self表示自定义图层，比如需要混合路线图等。天地图比较特殊，分6个目录，对应官网定义的6种地图。街道图底图是vec_w，街道图路网是cva_w，卫星图底图是img_w，卫星图路网是cia_w，地形图底图是ter_w，地形图路网是cta_w。**
...
```
2. 在pro中启用地图内核，比如百度地图内核对应需要在pro文件中增加一行定义  DEFINES += baidux，天地图内核对应 DEFINES += tianx
3. 引入头文件。
```cpp
#include "webview.h"
#include "maphelper.h"
...
1. 绘制一条直线，不断的做圆形路径移动。
1. 绘制一个扇形，不断的做圆形路径移动。
1. 绘制几个标注点对象，定义图标船形，不断的移动位置。
1. 右上角可以切换模拟的时间间隔，值越小速度越快。
1. 可以单击按钮来启动和停止模拟。
1. 支持缩放自动跟随，也就是地图变小这个图形也变小。
1. 可用来测试不断的变更覆盖物信息，会不会造成内存泄漏。
